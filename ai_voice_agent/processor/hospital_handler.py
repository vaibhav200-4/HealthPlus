"""
hospital_handler.py  –  Arav Mishra Hospital Assistant
...
def _normalize_hospital(raw: str) -> str | None:
    ...
    hospitals = get_hospitals() or []

    # Pass 1: Exact case-insensitive match against full hospital names
    for h_name, _ in hospitals:
        if lower_raw == h_name.lower().strip():
            return h_name

    # Pass 2: Exact match on explicit aliases
    for alias, h_name in HOSPITAL_ALIASES.items():
        if alias == lower_raw:
            return h_name

    # Pass 3: Substring match on explicit aliases
    for alias, h_name in HOSPITAL_ALIASES.items():
        if alias in lower_raw or (len(lower_raw) >= 4 and lower_raw in alias):
            return h_name
    
    # Pass 4: Fuzzy match against alias keys (SequenceMatcher ratio >= 0.85)
    best_alias_h = None
    ...
    # Pass 5: Exact match on meaningful hospital name tokens
    for h_name, _ in hospitals:
        h_tokens = [w for w in re.findall(r'\b[a-z0-9]+\b', h_name.lower()) if w not in GENERIC_HOSPITAL_TOKENS]
        h_meaningful = " ".join(h_tokens)
        if h_meaningful and raw_meaningful == h_meaningful:
            return h_name

    # Pass 6: Substring match on meaningful hospital name tokens
    for h_name, _ in hospitals:
        h_tokens = [w for w in re.findall(r'\b[a-z0-9]+\b', h_name.lower()) if w not in GENERIC_HOSPITAL_TOKENS]
        h_meaningful = " ".join(h_tokens)
        if h_meaningful and (h_meaningful in raw_meaningful or raw_meaningful in h_meaningful):
            return h_name

    # Pass 7: Fuzzy token similarity match
    best_h_name = None
    best_score = 0.0
    for h_name, _ in hospitals:
        h_tokens = [w for w in re.findall(r'\b[a-z0-9]+\b', h_name.lower()) if w not in GENERIC_HOSPITAL_TOKENS]
    ...
    def _handle_confirmation(self, intent_data: dict, user_text: str) -> str:
        ...
        if name_match:
            ...
            if new_name:
                ...
                fee = int(float(self.doctor_row[3])) if (self.doctor_row and len(self.doctor_row) > 3 and self.doctor_row[3]) else 0
                doc_disp = _fmt_doc(self.doctor_row) if self.doctor_row else (f"Dr. {self.doctor_name}" if self.doctor_name else "Doctor")
                return (f"Updated your name to {self.patient_name}. "
                        f"Please confirm your booking: {doc_disp} at {self.hospital_name}, "
                        f"on {self.appointment_date} at {self.appointment_time}, fee rupees {fee}, patient {self.patient_name}, number ending {num_ending}. "
                        "Shall I confirm this booking? Say yes or no.")
        ...
        if getattr(self, "user_id", None) and self.user_id != "00000000-0000-0000-0000-000000000001":
            try:
                ...
                if profiles and len(profiles) > 0:
                    profile = profiles[0]
                    raw_name = profile.get("name") or profile.get("full_name")
                    if raw_name:
